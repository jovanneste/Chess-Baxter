...
def main():
    ...
    pick_poses = ['00', '70', '07', '75', '77']
    place_poses = ['02', '73', '77', '66', '74']

    pick_piece_poses = list()
    place_piece_poses = list()

    for pick in pick_poses:
        p = positions[pick]
        pick_piece_poses.append(Pose(position = Point(x=p[0], y=p[1], z=p[2]), orientation = overhead_orientation))

    for place in place_poses:
        p = positions[place]
        place_piece_poses.append(Pose(position = Point(x=p[0], y=p[1], z=p[2]), orientation = overhead_orientation))

    for i in range(len(pick_poses)):
        if not rospy.is_shutdown():
            print("\nPicking...")
            pnp.pick(pick_piece_poses[i])
            print("\nPlacing...")
            pnp.place(place_piece_poses[i])
