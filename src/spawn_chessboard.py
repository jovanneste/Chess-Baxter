...
if __name__ == '__main__':
    ...
    board_setup = ['r******r', '***p****', '', '', '', '', '**p*****', 'R****B**']

    piece_positionmap = dict()
    piece_names = []

    starting_piece_pose = deepcopy(board_pose)
    starting_piece_pose.position.x = 0.5
    starting_piece_pose.position.y = 0.6
    starting_piece_pose.position.z = 0.78
    ...
    for row, each in enumerate(board_setup):
        ...
        for col, piece in enumerate(each):
            ...
            if piece in list_pieces:
                srv_call("%s%d" % (piece,col), pieces_xml[piece], "", starting_piece_pose, "world")
                piece_names.append("%s%d" % (piece,col))
                # pick and place the spawned piece 
                pnp.pick(Pose(position = Point(x=0.5, y=0.6, z=-0.14), orientation = overhead_orientation))
                pnp.place(Pose(position = Point(x=pose.position.x, y=pose.position.y, z=-0.13),orientation = overhead_orientation))
